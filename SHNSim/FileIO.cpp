...
bool FileIO::readSaveFileIntoSim()
{
	...
	for (size_t bs = 0; bs < numBSs; bs++)
	{
		...
		for (size_t ue = 0; ue < dbSize; ue++)
		{
			...
			auto newRecord = UERecord{ userID, mobilityID, Coord<float>{x, y}, ant, tr, snr, demand, bts, ps, rsrp, rsrq, ddr};
...
bool FileIO::appendLog(const uint32_t& simNum)
{
	...
	{
		log << "Time,BS_ID,BS_LOC_X,BS_LOC_Y,ANT_ID,ANT_SEC,TRX_ID,TRX_X,TRX_Y,TRX_ANG,UE_ID,UE_MID,UE_LOC_X,UE_LOC_Y,MAX_DR,DEMAND_DR,REAL_DR,BS_Tran_SNR,UE_Rec_SNR,RSRP,RSRQ,DDR\n"; // added the name for the column holding the bs number
