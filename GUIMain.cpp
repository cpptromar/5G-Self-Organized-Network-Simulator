...
static void on_location_selected(GtkComboBox* combo, gpointer user_data)
{
	gchar* selected_location = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(combo));

	//PORT for api call
	//some_other_function(selected_profile);

	//free's the memory allocated for the selected profile.
	g_free(selected_location);
}

GtkTreeModel* create_profiles_model()
{
	GtkListStore* store;
	GtkTreeIter iter;
	...
	for (i = 0; i < 3; i++)
	{
		...
		gtk_list_store_set(store, &iter, 0, profiles[i], -1);
	}
...
void setUpDrawingWindow()
{
	...
	location_menu = gtk_combo_box_text_new();

	const char* states[] = {
		"Engineering Building", "Sports Stadium", "Library"
	};

	for (int i = 0; i < 3; i++)
	{
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(location_menu), states[i]);
	}
	*/
	...
	// Create the dropdown for Profiles
	GtkWidget* profiles_combo = gtk_combo_box_new_with_model(create_profiles_model());
	GtkCellRenderer* renderer = gtk_cell_renderer_text_new();
	gtk_combo_box_set_title(GTK_COMBO_BOX(profiles_combo), "Select a time of day");
	gtk_combo_box_set_active(GTK_COMBO_BOX(profiles_combo), 0); // Set default active item


	// gtk_box_pack_start(GTK_COMBO_BOX(states_combo), renderer, TRUE); //potentiall erroneous
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(profiles_combo), renderer, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(profiles_combo), renderer, "text", 0, NULL);
	// Place the states_combo widget in your GUI, for example, adding it to realTimeParamBox:
	gtk_box_pack_start(GTK_BOX(realTimeParamBox), profiles_combo, 0, 0, 5);
	//gtk_box_pack_start(GTK_BOX(realTimeParamBox), locations_menu, 0, 0, 5);

	//gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(location_menu), renderer, TRUE);
	//gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(location_menu), renderer, "text", 0, NULL);
	...
	//g_signal_connect(location_menu, "changed", G_CALLBACK(on_state_selected), NULL);

	//gtk_combo_box_set_wrap_width(GTK_COMBO_BOX(location_menu), 1);
}
...
void setUpSimParamWindow()
{
	...
	GtkWidget* profiles_combo = gtk_combo_box_new_with_model(create_profiles_model());
	GtkCellRenderer* renderer = gtk_cell_renderer_text_new();
	gtk_combo_box_set_title(GTK_COMBO_BOX(profiles_combo), "Select a profile");
	gtk_combo_box_set_active(GTK_COMBO_BOX(profiles_combo), 0); // Set default active item
	...
	simLength = gtk_label_new("Length of Simulation/Profiles");
	...
	// ================= Profiles ComboBox
	comboLabel = gtk_label_new("Selected Profile");
	verCombo = gtk_combo_box_text_new(); //https://developer.gnome.org/gnome-devel-demos/stable/combobox.c.html.en
	const char* profiles[] = { "Morning", "Afternoon", "Evening" };	//list the available algorithm versions... ORDER MATTERS!!!!
	for (int i = 0; i < sizeof(profiles) / sizeof(profiles[0]); i++)	//the division of sizeof()/sizeof() simply returns how many elements in array
		gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(verCombo), profiles[i]);
	gtk_combo_box_set_active(GTK_COMBO_BOX(verCombo), 1);	//set [1] to be default
	g_signal_connect(verCombo, "scroll-event", G_CALLBACK(stopScroll), NULL);	//this stops scroll wheel from changing the value [which can accidentally happen]
...
static void button_clicked15(GtkWidget* widget, gpointer data)
{
	GUIDataContainer::AttractivenessArray[GUIDataContainer::selectedTile] = stoi(gtk_entry_get_text(GTK_ENTRY(data)));
}

static void button_clicked16(GtkWidget* widget, gpointer data)
{
	GUIDataContainer::AttractivenessArray[GUIDataContainer::selectedTile] = stoi(gtk_entry_get_text(GTK_ENTRY(data)));
}

static void button_clicked17(GtkWidget* widget, gpointer data)
{
	GUIDataContainer::AttractivenessArray[GUIDataContainer::selectedTile] = stoi(gtk_entry_get_text(GTK_ENTRY(data)));
}
	...
	if (event->button == 1) //Left Mouse Click
	{
		...
		if (!changedTile)
		{
			...
			{	
				...
				GtkWidget* buttonMorningProfile, * buttonAfternoonProfile, * buttonEveningProfile;
				GtkWidget* comboLabel1;
				GtkWidget* verCombo1;
				//create cellparameters, define as a new window
				GtkWidget* cellparameters = gtk_window_new(GTK_WINDOW_TOPLEVEL);
				gtk_container_set_border_width(GTK_CONTAINER(cellparameters), 15);
				...
				gtk_grid_set_row_spacing(GTK_GRID(gridCellParameters), 30);
				gtk_grid_set_column_spacing(GTK_GRID(gridCellParameters), 30);
				gtk_grid_set_column_homogeneous(GTK_GRID(gridCellParameters), TRUE);

				comboLabel1 = gtk_label_new("Location");
				verCombo1 = gtk_combo_box_text_new(); //https://developer.gnome.org/gnome-devel-demos/stable/combobox.c.html.en
				const char* locations[] = { "Engineering Building", "Stadium", "Library" };	//list the available algorithm versions... ORDER MATTERS!!!!
				for (int i = 0; i < sizeof(locations) / sizeof(locations[0]); i++)	//the division of sizeof()/sizeof() simply returns how many elements in array
					gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(verCombo1), locations[i]);
				gtk_combo_box_set_active(GTK_COMBO_BOX(verCombo1), 0);	//set [1] to be default
				g_signal_connect(verCombo1, "scroll-event", G_CALLBACK(stopScroll), NULL);	//this stops scroll wheel from changing the value [which can accidentally happen]
				...
				buttonMorningProfile = gtk_button_new_with_label("Morning Profile");
				buttonAfternoonProfile = gtk_button_new_with_label("Afternoon Profile");
				buttonEveningProfile = gtk_button_new_with_label("Evening Profile");
				...
				gtk_grid_attach(GTK_GRID(gridCellParameters), verCombo1, 0, 4, 2, 1);
				gtk_grid_attach(GTK_GRID(gridCellParameters), buttonMorningProfile, 0, 5, 2, 1);
				gtk_grid_attach(GTK_GRID(gridCellParameters), buttonAfternoonProfile, 0, 6, 2, 1);
				gtk_grid_attach(GTK_GRID(gridCellParameters), buttonEveningProfile, 0, 7, 2, 1);
				...
				g_signal_connect(buttonMorningProfile, "clicked", G_CALLBACK(button_clicked15), entryDensity);
				g_signal_connect(buttonAfternoonProfile, "clicked", G_CALLBACK(button_clicked16), entryDensity);
				g_signal_connect(buttonEveningProfile, "clicked", G_CALLBACK(button_clicked17), entryDensity);
