...
bool Simulator::moveUE(const size_t& bsOrigin, const size_t& UE_ID, const Coord<float>& newloc)
{
	...
	//adds number of users at the start of the afternoon profile and removes users at the start of the evening profile
	if (Simulator::envClock == Simulator::simulationLength / 3) {
		const auto newLoc = Coord<float>{ loc.x + bs.getLoc().x, loc.y + bs.getLoc().y };
		auto newUser = UserEquipment{ newLoc, cUserID, currMobilityID, possMaxDrsForUE, currentDemand };
		Simulator::addUE(newUser);
	}
	else if (Simulator::envClock == (2 * simulationLength) / 3) {
		const auto antenna = this->userRecords.removeUE(UE_ID);
		auto newUser = UserEquipment{ newLoc, cUserID, currMobilityID, possMaxDrsForUE, currentDemand };
		if (newUser.antenna < this->BSAntennae.size())
			return this->BSAntennae[antenna].getConnectionInfo_m().removeUE(UE_ID);
		else
			return false;
	}
}
